...
runall_result test_main()
{
    ...
    Log() << "Creating a section [0-16) and reinterpreting as structs" << std::endl;
    array_view<Foo, 2> av_struct = av.view().section(extent<1>(16)).reinterpret_as<Foo>().view_as(extent<2>(2, 4));
    
    Log() << "Creating a section [14-30) of 3d uint" << std::endl;
    ...
    Log() << "Now performing implic synch on elements [14-30)" << std::endl;
    av_uint[index<3>()];
    
    Log() << "Elements [14-16) of the underlying data should have changed" << std::endl;
    
    if (av.data()[14] != 1 || av.data()[15] != 2)
    {
        Log(LogType::Error) << "Underlying data was not updated when it shouldn't have been" << std::endl;
        Log(LogType::Error) << "av.data()[14] = " << av.data()[14] << std::endl;
        Log(LogType::Error) << "av.data()[15] = " << av.data()[15] << std::endl;
        return runall_fail;
    }
