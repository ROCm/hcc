...
#define PRIORITY_OFFSET 2
    ...
    void init(void *data, int count, bool UseHost) {
        if (count > 0) {
            cl_int err;
            cl_mem dm;
            if (UseHost)
                dm = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR,
                                    count, data, &err);
            else {
                dm = clCreateBuffer(context, CL_MEM_READ_WRITE, count, nullptr, &err);
