...
struct projection_helper<T, 1>
{
    ...
    static result_type project(vector<T, 1>& now, int i) __CPU__ __HC__ {
#if __KALMAR_ACCELERATOR__ != 1
        now.cache.get_cpu_access(true);
#endif
        T *ptr = reinterpret_cast<T *>(now.cache.get() + i + now.offset + now.index_base[0]);
    ...
    static result_type project(const vector<T, 1>& now, int i) __CPU__ __HC__ {
#if __KALMAR_ACCELERATOR__ != 1
        now.cache.get_cpu_access(true);
#endif
        T *ptr = reinterpret_cast<T *>(now.cache.get() + i + now.offset + now.index_base[0]);
...
class vector : public std::vector<T> {
 ...
 public:
  ...
  vector(vector&& that)  __CPU__ __HC__
    : std::vector<T>(that), offset(that.offset){
    if(this != &that){
      this->cache = acc_buffer_t(that.size(), that.data());
      extent = hc::extent<N>(that.size());
      extent_base = extent;
    }
  }
  vector(vector& that) __CPU__ __HC__
  ...
  vector& operator=(const vector&& other) __CPU__ __HC__  = delete;
  vector& operator=(const vector& other) __CPU__ __HC__ = delete;
  ...
  const T&
  operator[] (int i) const __CPU__  {
      return std::vector<T>::operator[](i);
  }
  T&
  operator[] (int i)  __CPU__  {
      return std::vector<T>::operator[](i);
  }
