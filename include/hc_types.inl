...
private:
  ...
  static constexpr float unorm_zero = 0.0f;

  static constexpr float norm_zero = 0.0f;
  static constexpr float norm_min  = -1.0f;
  static constexpr float norm_max  = 1.0f;

  float clamp(float v) {
    constexpr float max = isSigned?norm_max:unorm_max;
    constexpr float min = isSigned?norm_min:unorm_min;
