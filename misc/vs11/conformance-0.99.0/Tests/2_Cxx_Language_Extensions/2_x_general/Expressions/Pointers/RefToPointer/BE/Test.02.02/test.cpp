...
void run_mykernel(vector<type> &a, vector<type> &b, vector<type> &c, vector<type> &fa1, vector<type> &fa2, vector<type> &fa3, vector<type> &fa4, vector<int> &flag, accelerator_view av)
{
    ...
    parallel_for_each(av, a_a.extent.tile<BLOCK_SIZE_1D, BLOCK_SIZE_1D, BLOCK_SIZE_1D>(), [=] (tiled_index<BLOCK_SIZE_1D, BLOCK_SIZE_1D, BLOCK_SIZE_1D> idx) mutable __GPU_ONLY {
        ...
        sab<type> *pab = b1 ? (--b2, &oabf1) : &oab; // b2 is still 0.
        sc<type> *pc = b1 ? (--b2, &ocf1) :  &oc; // b2 is still 0.
        ...
        k::func(idx, (b3 % b2) ?  pabf1 : pab , pabf1, pabf2, b2 ? pc : pcf1, pcf1, pcf2, a_flag, b1, b2, b3, b4);
    });
...
runall_result test_main()
{
    ...
    accelerator_view av = require_device_for<AMP_ELEMENT_TYPE>().default_view;
