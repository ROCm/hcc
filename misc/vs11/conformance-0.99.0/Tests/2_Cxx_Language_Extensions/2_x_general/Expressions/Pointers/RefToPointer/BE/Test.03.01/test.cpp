    ...
    vector<int> &flag, accelerator_view av)
{
    ...
    parallel_for_each(av, a_a.extent.tile<BLOCK_SIZE>(), [=] (tiled_index<BLOCK_SIZE> idx) mutable __GPU_ONLY {
        ...
        array_view<type> *pa = b3 ? &ara[1] : (--b2, &araf1[0]);
        array_view<type> *paf1 = &araf1[0];
        array_view<type> *paf2 = &araf2[1];
        array_view<int> *pb = b2 ? &arb[2] : &arbf1[0];
        ...
        k::func(idx, 
            b1? (--b2, paf1) : pa, b2 ? pb : pbf1, paf1, pbf1,
            av_flag, b1, b2, b3, b4);
    });
