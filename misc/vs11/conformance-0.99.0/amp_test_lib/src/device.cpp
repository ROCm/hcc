...
#pragma endregion
			...
			inline device_bit_flags compute_device_bit_flags(const accelerator& device) {
				...
				const wstring& dpath = device.device_path;
				const wstring& ddesc = device.description;
				...
				if(device.supports_limited_double_precision) {
...
#pragma region device predicates
			...
			inline bool is_amptest_device_allowed(const device_info& dinfo) {
				...
				if(has_bits_set(dinfo.first, device_bit_flags::IS_GPU) && dmf_allowed_gpu_device_paths.size() > 0) {
					const wstring& dpath = dinfo.second.device_path;
			...
			std::string retrieved_device_caps_to_string(const accelerator& device) {
				...
				} else if(device.supports_limited_double_precision) {
			...
			bool in_priority_order(const device_info& dev1, const device_info& dev2) {
				const wstring& dpath1 = dev1.second.device_path;
				const wstring& dpath2 = dev2.second.device_path;

				if(dev1.second.is_emulated != dev2.second.is_emulated) {
					return !dev1.second.is_emulated;
				} else if(dpath1 == dpath2) {
					return true;
				} else if(dev1.second.is_emulated) {
					...
					if(dev1.second.supports_double_precision != dev2.second.supports_double_precision) {
						return dev1.second.supports_double_precision;
					} else if(dev1.second.supports_limited_double_precision != dev2.second.supports_limited_double_precision) {
						return dev1.second.supports_limited_double_precision;
					} else if(dev1.second.has_display != dev2.second.has_display) {
						return !dev1.second.has_display;
					} else {
						return dev1.second.dedicated_memory > dev2.second.dedicated_memory;
					}
				}
			}
				...
				if(dmf_known_IHV_failure_behavior == known_IHV_failure_behavior::EXCLUDE_DEVICE
					...
					newLast = remove_if(avail_infos.begin(), newLast, [](device_info dinfo) {
						if(is_device_known_IHV_failure(dinfo.first)) {
							Log_writeline(LogType::Warning, "DMF:    excluding knownfail IHV device: %ws (%s)", dinfo.second.description.c_str(), retrieved_device_type_to_string(dinfo.second).c_str());
							return true;
						} else {
							return false;
						}
		...
		std::vector<accelerator> AMP_TEST_API get_available_devices(device_flags required_flags) {
			...
			if(details::dmf_known_IHV_failures.size() != 0) {
				...
				newLast = remove_if(avail_infos.begin(), newLast, [&knownfail_count](details::device_info dinfo) {
					...
					Log_writeline(LogType::Warning, msg_format, dinfo.second.description.c_str(), details::retrieved_device_type_to_string(dinfo.second).c_str());
        ...
        accelerator AMP_TEST_API require_device(const accelerator& excluded_device, device_flags required_flags) {
			...
			excluded_device_paths[0] = excluded_device.device_path;
			...
			bool does_device_match(const accelerator& device, device_flags required_flags) {
				...
				if(dpath == accelerator::cpu_accelerator) {
					...
					dev_type_match = false
						|| (has_bits_set(required_flags, device_flags::EMULATED) && device.is_emulated)
						|| (has_bits_set(required_flags, device_flags::NOT_EMULATED) && !device.is_emulated)
				...
				if(has_any_bits_set(required_flags, details::device_capability_flags_mask)) {
					dev_caps_match = false
						|| (has_bits_set(required_flags, device_flags::DOUBLE) && device.supports_double_precision)
						|| (has_bits_set(required_flags, device_flags::LIMITED_DOUBLE) && device.supports_limited_double_precision)
						|| (has_bits_set(required_flags, device_flags::LIMITED_DOUBLE_ONLY)
							&& device.supports_limited_double_precision
							&& !device.supports_double_precision
							)
						|| (has_bits_set(required_flags, device_flags::NO_DOUBLE)
							&& !device.supports_limited_double_precision
							&& !device.supports_double_precision
