...
namespace Concurrency
{
	namespace Test
	{
		...
		amptest_context_t::amptest_context_t() :
			...
			_cerr_logfile_path(L""),
			_cout_logfile_path(L""),
		...
		const std::wstring& amptest_context_t::get_stderr_logfile_path() const 
		{ 
			return _cerr_logfile_path;
		}

		const std::wstring& amptest_context_t::get_stdout_logfile_path() const 
		{ 
		...
		void amptest_context_t::set_stderr_logfile_path(const std::wstring& stderr_filename) 
		{ 
		...
		void amptest_context_t::set_stdout_logfile_path(const std::wstring& stdout_filename) 
		{ 
...
#pragma warning(default:4996)
			...
			{	
				std::wstring name_w = convert_to_wchar_t(name);
				auto val = _env_cache.find(name_w);
				if (val != _env_cache.end())
				{
					val_str = convert_to_utf8(val->second);
				}
			}
		...
		int amptest_context_t::load_environment_variable_cache_from_file(const std::wstring& file_path)
		{
			int count = 0;
			...
			while(infile.is_open() && !infile.eof())
			{
				...
				std::wstring in_w = convert_to_utf16(in);

				size_t index = in_w.find_first_of(L'=');
				if (index != std::wstring::npos)
				{

					std::wstring key = in_w.substr(0, index);
					std::wstring value;

					// If '=' is not the last character, then do the substr() operation and trim whitespace.
					if (in_w.size() > (index + 1) )
					{
						value = trim(in_w.substr(index+1));
					}
		...
		int amptest_context_t::dump_environment_variable_cache() const
		{
			...
			std::for_each(_env_cache.begin(), _env_cache.end(), [&](std::pair<std::wstring, std::wstring> value)
						  {
