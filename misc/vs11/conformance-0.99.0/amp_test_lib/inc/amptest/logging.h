...
namespace Concurrency
{
    namespace Test
    {
		...
		std::string AMP_TEST_API get_type_name(const type_info& ti);
		...
		// Create a manipulator for formatting a value as code w/o creating an intermmediate string representation
		// See: http://msdn.microsoft.com/en-us/library/b5858a50(v=vs.110).aspx
		template <typename T>
		inline std::_Smanip<T> format_as_code(const T& v) {
			return std::_Smanip<T>(details::stream_value_as_code_wrapper<T>, v);
		}

		template <typename T>
		inline std::_Smanip<T> format_as_hex(const T& v) {
			return std::_Smanip<T>(details::stream_value_as_hex_wrapper<T>, v);
		}
