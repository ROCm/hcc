    ...
    namespace Test
    {
		...
		bool VerifyDataOnCpu(const _amp_container_type_1<_type, _rank>& actual, const _amp_container_type_2<_type, _rank>& expected, _type diff = 0)
		{
			if(actual.extent != expected.extent)
			{
				...
				Log(LogType::Error) << "Actual: " << actual.extent << " Expected: " << expected.extent;
				return false;
			}
			
			std::vector<_type> vect_actual(actual.extent.size());
			copy(actual, vect_actual.begin());

			std::vector<_type> vect_expected(expected.extent.size());
		...
		{
			if(actual.extent.size() != expected.size())
			{
				Log(LogType::Error) << "Size of actual and expected does not match.\n";
				Log(LogType::Error) << "Size of actual : " << actual.extent.size();
				Log(LogType::Error) << "Size of expected : " << expected.size();
				return false;
			}
			
			std::vector<_type> temp_cont(actual.extent.size());
		...
		{
			if(expected.extent.size() != actual.size())
			{
				Log(LogType::Error) << "Size of expected and actual does not match.\n";
				Log(LogType::Error) << "Size of actual: " << actual.size();
				Log(LogType::Error) << "Size of expected: " << expected.extent.size();
				return false;
			}
			
			std::vector<_type> temp_cont(expected.extent.size());
        ...
        bool VerifyDataOnAcc(array<_type, _rank>& actual, array<_type, _rank>& expected, array<_type, _rank>& stagingArrResult, _type diff = 0)
        {
            ...
            if(stagingArrResult.extent != expected.extent)
            {        
                Log(LogType::Error) << "Grid value for result staging array and input containers does not match.";
                Log(LogType::Error) << "Input containers: " << actual.extent << " staging array result: " << expected.extent;
                return false;
            }
            
            parallel_for_each(actual.extent, [&actual, &expected, &stagingArrResult](index<_rank> idx) restrict(amp)
            {
		...
		{
			std::vector<_type> vect1(inputArrayView.extent.size());
        ...
        int VerifyAllSameValue(const std::vector<_type>& inputVector, const _type& value)
        {
            ...
            std::vector<_type>::const_iterator iter = std::find_if_not(inputVector.begin(), inputVector.end(), [&](_type el) -> bool {
