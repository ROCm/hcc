...
#
# you will also need the 32-BIT VERSIONS of the following packages
...
# - SciPy 0.9.0 (32-bit, 64-bit unofficial, supports Python 2.4 - 2.7 or 3.1 - and NumPy 1.5 or greater) - http://sourceforge.net/projects/scipy/files/scipy/
# - matplotlib 1.0.1 (32-bit & 64-bit, supports Python 2.4 - 2.7) - http://sourceforge.net/projects/matplotlib/files/matplotlib/
#
# please note that you should install all 4 packages above in the order that they are listed
...
def plotFromDataFile():
  ...
  for thisFile in args.datafile:
    ...
    for row in resultsContents:
      row = row.split(',')
      row = TableRow(TestCombination(row[0],row[1],row[2],row[3]), row[4])
      data.append( GraphPoint(row.parameters.x, row.parameters.device, row.parameters.precision, row.parameters.label, row.gflops) )
...
parser = argparse.ArgumentParser(description='Plot performance of the Bolt\
    library. plotPerformance.py reads in data tables from Bolt.\
...
fileOrDb.add_argument('-d', '--datafile',
  ...
  measurePerformance.py. may be used multiple times to indicate\
  multiple input files. e.g., -d cypressOutput.txt -d caymanOutput.txt')
parser.add_argument('-x', '--x_axis',
  dest='graphxaxis', default='x', choices=xaxisvalues,
  help='indicate which value will be represented on the x axis.' )
