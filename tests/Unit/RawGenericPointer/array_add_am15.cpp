...
template<typename T>
[[hc]] static void setC(T a, T b, T* c, int idx) {
  c[idx] = a + b;
}
...
template<typename T>
[[hc]] static T getB(T* a, T* b, T* c, int idx) {
  return b[idx];
}

template<typename T>
[[hc]] static T getA(T* a, T* b, T* c, int idx) {
  return a[idx];
}
...
bool test() {
  ...
  auto k = [=](hc::index<1> idx) [[hc]] {
    T a = Foo::getA(data1_d, data2_d, data3_d, idx[0]);
    T b = Foo::getB(data1_d, data2_d, data3_d, idx[0]);
    Bar::setC(a, b, data3_d, idx[0]);
  };
