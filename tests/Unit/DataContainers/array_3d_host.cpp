...
void init3D(std::vector<int>& vec) {
  for (int i = 0; i < N0; ++i) {
    for (int j = 0; j < N1; ++j) {
      for (int k = 0; k < N2; ++k) {
...
TEST(ClassArray3D, ConstructorExtentIterator) {
  std::vector<int> v;
  init3D(v);
  Concurrency::extent<3> ext(N0, N1, N2);
  Concurrency::array<int, 3> arr1(ext, v.begin());
  EXPECT_EQ(ext, arr1.extent);
  for (int i = 0; i < N0; i++)
    for (int j = 0; j < N1; j++)
      for (int k = 0; k < N2; k++)
        EXPECT_EQ(v[i*N1*N2+j*N2+k], arr1(i, j, k));
}

TEST(ClassArray3D, ConstructorSizesIterator) {
  std::vector<int> v;
  init3D(v);
  Concurrency::extent<3> ext(N0, N1, N2);
  Concurrency::array<int, 3> arr1(N0, N1, N2, v.begin());
  EXPECT_EQ(ext, arr1.extent);
  for (int i = 0; i < N0; i++)
    for (int j = 0; j < N1; j++)
      for (int k = 0; k < N2; k++)
...
TEST(ClassArray3D, ConstructorSizes) {
  Concurrency::extent<3> ext(N0, N1, N2);
  Concurrency::array<int, 3> arr1(N0, N1, N2);
  EXPECT_EQ(ext, arr1.extent);
}
#if 0
