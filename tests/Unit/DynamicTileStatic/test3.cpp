...
bool test1D() {
  ...
  // set dynamic tile size as 0 for now as we don't test this feature in this test yet
  hc::ts_allocator tsa;
  hc::parallel_for_each(hc::tiled_extent_1D(grid_size, tile_size), tsa, [=](hc::tiled_index_1D& idx) restrict(amp) {
    av5(idx) = idx.global[0];
    av6(idx) = idx.local[0];
    av7(idx) = idx.tile[0];
    av8(idx) = idx.tile_origin[0];
...
bool test2D() {
  ...
  // set dynamic tile size as 0 for now as we don't test this feature in this test yet
  hc::ts_allocator tsa;
  hc::parallel_for_each(hc::tiled_extent_2D(grid_size_0, grid_size_1, tile_size_0, tile_size_1), tsa, [=](hc::tiled_index_2D& idx) restrict(amp) {
    av9(idx) = idx.global[0];
    av10(idx) = idx.global[1];
    av11(idx) = idx.local[0];
    av12(idx) = idx.local[1];
...
bool test3D() {
  ...
  // set dynamic tile size as 0 for now as we don't test this feature in this test yet
  hc::ts_allocator tsa;
  hc::parallel_for_each(hc::tiled_extent_3D(grid_size_0, grid_size_1, grid_size_2, tile_size_0, tile_size_1, tile_size_2), tsa, [=](hc::tiled_index_3D& idx) restrict(amp) {
    av13(idx) = idx.global[0];
    av14(idx) = idx.global[1];
    av15(idx) = idx.global[2];
    av16(idx) = idx.local[0];
...
int main() {
  ...
  ret &= test1D_new<1,1>();
  ret &= test1D_new<32, 4>();
  ret &= test1D_new<1024, 16>();
  ret &= test1D_new<4096, 256>();

  ret &= test2D_new<1, 1, 1, 1>();
  ret &= test2D_new<8, 8, 2, 2>();
  ret &= test2D_new<1024, 16, 32, 4>();
  ret &= test2D_new<4096, 256, 16, 16>();

  ret &= test3D_new<1, 1, 1, 1, 1, 1>();
  ret &= test3D_new<8, 8, 8, 2, 2, 2>();
  ret &= test3D_new<1024, 32, 16, 32, 4, 2>();
