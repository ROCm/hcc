...
test_atomicrmw_nand:
  ...
  %nadd1 = atomicrmw nand i32* %0, i32 %1 monotonic
;CHECK: atomicnoret_and_global_rlx_sys_equiv({{.*}})_b32 {{.*}}

  %nadd2 = atomicrmw nand i32* %0, i32 %1 acquire
;CHECK: atomicnoret_and_global_scacq_sys_equiv({{.*}})_b32 {{.*}}

  %nadd3 = atomicrmw nand i32* %0, i32 %1 release
;CHECK: atomicnoret_and_global_screl_sys_equiv({{.*}})_b32 {{.*}}

  %nadd4 = atomicrmw nand i32* %0, i32 %1 acq_rel
;CHECK: atomicnoret_and_global_scar_sys_equiv({{.*}})_b32 {{.*}}

  %nadd5 = atomicrmw nand i32* %0, i32 %1 seq_cst
;CHECK: atomicnoret_and_global_scar_sys_equiv({{.*}})_b32 {{.*}}
