...
;CHECK: atomicnoret_max_global_rlx_sys_equiv({{.*}})_s32 {{.*}}

  %max2 = atomicrmw max i32* %0, i32 %1 acquire
;CHECK: atomicnoret_max_global_scacq_sys_equiv({{.*}})_s32 {{.*}}

  %max3 = atomicrmw max i32* %0, i32 %1 release
;CHECK: atomicnoret_max_global_screl_sys_equiv({{.*}})_s32 {{.*}}

  %max4 = atomicrmw max i32* %0, i32 %1 acq_rel
;CHECK: atomicnoret_max_global_scar_sys_equiv({{.*}})_s32 {{.*}}
