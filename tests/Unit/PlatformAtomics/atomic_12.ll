...
test_atomicrmw_min:
  ...
  %min1 = atomicrmw min i32* %0, i32 %1 monotonic
;CHECK: atomicnoret_min_global_rlx_sys_equiv({{.*}})_s32 {{.*}}

  %min2 = atomicrmw min i32* %0, i32 %1 acquire
;CHECK: atomicnoret_min_global_scacq_sys_equiv({{.*}})_s32 {{.*}}

  %min3 = atomicrmw min i32* %0, i32 %1 release
;CHECK: atomicnoret_min_global_screl_sys_equiv({{.*}})_s32 {{.*}}

  %min4 = atomicrmw min i32* %0, i32 %1 acq_rel
;CHECK: atomicnoret_min_global_scar_sys_equiv({{.*}})_s32 {{.*}}

  %min5 = atomicrmw min i32* %0, i32 %1 seq_cst
;CHECK: atomicnoret_min_global_scar_sys_equiv({{.*}})_s32 {{.*}}
