...
bool test_array_rank(int extval = _rank)
{
    ...
    parallel_for_each(e, [&](index<_rank> idx) restrict(amp,cpu){
        a1[idx] = 1;
    });
...
bool test_feature_itr(extent<_rank> _extent, _Iterator _first, _Iterator _last)
{
    ...
    parallel_for_each(src1.get_extent(), [&] (index<_rank> idx) restrict(amp)
    {
        src1(idx) = src1[idx]*2;
    });
    ...
    parallel_for_each(src2.get_extent(), [&] (index<_rank> idx) restrict(amp)
    {
    ...
    parallel_for_each(dst.get_extent(), [&] (index<_rank> idx) restrict(amp)
    {
...
bool test_feature_itr(extent<_rank> _e, _BeginIterator _first)
{
    ...
    parallel_for_each(src.get_extent(), [&] (index<_rank> idx) restrict(amp)
    {
        dst[idx] = src(idx);
    });
...
bool test_accl_constructor(_accl device)
{
    ...
    if (_rank > 0) // for rank 1
    {
        ...
        // let the kernel initialize data;
        extent<1> e1(edata);
        parallel_for_each(e1, [&](index<rank> idx) restrict(amp)
        {
            src[idx] = _rank;
        });
...
bool test_accl_staging_buffer_constructor(_staging device1, _staging device2)
{
    ...
    { // Extent based staging constructor
        ...
        passed &= (VerifyAllSameValue(opt, static_cast<_type>(_rank)) == -1);
    }
