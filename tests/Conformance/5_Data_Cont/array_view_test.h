...
namespace Test
{
    ...
    public:
        ...
        _view(extent, *_data.get())
        {
            ...
            std::ostream_iterator<value_type> os_iter(std::cout, ", ");
        ...
        ///<summary>uses the known-values to verify the underlying data, then returns 0</summary>
        int pass()
        {
        ...
        ///<summary>logs information, then returns 1</summary>
        int fail()
        {
            for(auto iter = _known_values.get()->begin(); iter != _known_values.get()->end(); iter++)
            {
